...
All the directive parameters are mandatory. You can create several heaps, but only one '//=>\s*hashfunc' directive is necessary.
...
The '//=>\s*hashfunc' directive names the MurMurHash function that is necessary for the process.
...
while (<$input_file>) {
    if (/^\/\/=>\s*alloc\s+(.+)\s*,\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*,\s*([a-zA-Z_][a-zA-Z0-9_]*),\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*$/) {
    ...
    } elsif (/^\/\/=>\s*hashfunc\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*$/) {
